...
MyRng::MyRng()  /* default constructor */
{
  ...
  /*Change this based on your generator needs*/
  nvectors = 0;
...
/*      of gennum in [0,total_gen) each call                              */
  ...
  /*** Add initialization statements for your data in the vectors and other 
...
int MyRng::pack_rng(char **buffer)
{
  ...
  {
    p += store_intarray(vectors[i], vectors[i].size(), 4, p);
  }
...
int MyRng::unpack_rng( char *packed)
{
  ...
  for (i = 0; i < vectors.size(); ++i)
  {
    p += load_intarray(p, vectors[i].size(), 4, vectors[i]);
  }
