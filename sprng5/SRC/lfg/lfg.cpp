...
int LFG::pack_rng(char **buffer)
{
  ...
  size = 4 + (3*(lval)+5)*4 + 4;// strlen(gentype)+1;
...
/*      check values of parameters for consistency                       */
  ...
  */
  ...
  p += load_intarray(p,length-1,4,si);
  p += load_intarray(p,length,4,r0);
  p += load_intarray(p,length,4,r1);
